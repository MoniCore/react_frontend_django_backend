...
def getgroups(tempgroups,request,status):
    ...
    for item in tempgroups:
        ...
        if thisUser.avatar:
            ...
            data['avatar'] = '/static/images/avatar.png'
...
def create(request):
    ...
    curuser = get_curuser(request,user.id) 
    users = get_users(request)  
    users_for_invite = CustomUser.objects.exclude(id=user.id)
    return render(request,'groups/create.html',{'users':users,'curuser':curuser,'users_for_invite':users_for_invite}) 
...
def view_group_detail(request,group_id):
    ...
    thisUser = CustomUser.objects.get(id=cur_temp.user_id)
    if cur_temp.avatar:
        cur_group['back'] = cur_temp.avatar.url
    else:
        cur_group['back'] = '/static/images/no_banner.png'
    ...
    cur_group['username'] = thisUser.first_name + " " + thisUser.last_name
    ...
    if this_user.avatar:
        ...
        cur_group['avatar'] = '/static/images/avatar.png'
    ...
    users = get_users(request)  
    users_for_invite = CustomUser.objects.exclude(id=user.id)
    return render(request,'groups/details.html',{'users':users,'curuser':curuser,'users_for_invite':users_for_invite,'cur_group':cur_group,'groups':allgroups}) 
